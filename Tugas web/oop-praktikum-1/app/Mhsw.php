<?php
 ...
 class dosen extends Peserta {
    ...
    $this->db = new PDO("mysql:host=localhost;dbname=dbakademik", "root", ""); } catch (PDOException $e) { die ("Error " . $e->getMessage());
       ...
       public function simpan()
       {
           $sql = "insert into tb_dosen values ('','".$_GET['nim']."','".$_GET['nama']."','".$_GET['alamat']."')";
           $stmt = $this->db->prepare($sql);
           $stmt->execute();
           echo "DATA BERHASIL DISIMPAN !";
       } 
       ...
       {
           $sql = "update tb_dosen set dosen_nim='".$nim."', dosen_nama='".$nama."', dosen_alamat='".$alamat."' where dosen_id='".$id."'";
           $stmt = $this->db->prepare($sql);
           $stmt->execute();
           echo "DATA BERHASIL DIUPDATE !";
       } 
 ...
 $this->db = new PDO("mysql:host=localhost;dbname=dbakademik", "root", ""); } catch (PDOException $e) { die ("Error " . $e->getMessage());
    ...
    {
        $sql = "SELECT * FROM tb_prodi where id_dosen ='".$_GET['id_dosen']."'";
        $stmt = $this->db->prepare($sql);
        $stmt->execute();
        $data = [];
        while ($rows = $stmt->fetch()) {
